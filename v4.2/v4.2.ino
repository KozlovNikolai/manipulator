...
#define BatPIN A0                 //battery monitor arduino number
#define VERSION "4.2"
...
#define WPM CPM/5
#define LevelBatt1 400
#define LevelBatt2 520
...
int maxValue[maxMenu+1] = {0,  600,             4,                 2,      2,       3,            400,           2,       1     };   
...
void doPrint() {
  switch (menuCount){
    ...
    case 5: LD.printString_12x16 ("AutoProbel", 0, 0);LD.printString_12x16 ("          ", 0, 2);if(modeAutoProbel==1){LD.printString_12x16("5 symbols ", 0, 2);};
                                                                                                if(modeAutoProbel==2){LD.printString_12x16("7 tochek  ", 0, 2);};
                                                                                                if(modeAutoProbel==3){LD.printString_12x16(" Off      ", 0, 2);};break;
...
void doMenu(){
  ...
  switch(doEncButton()){
    ...
    case 2: if(menuCount==8){
            ...
            LD.printString_12x16 ("SAVING OK!", 0, 4);delay(1000);LD.printString_12x16 ("          ", 0, 4); 
                            };
...
void doAutoProbel(){
  switch (modeAutoProbel) {
                        case 1: if (WordCount==5){if(modeKeyboard==1){Keyboard.print(" ");};WordCount=0;};break;
                        case 2: if (((micros() - stopFlow) > (sizeDot * 7)) && MorseSignal == 0 && WordCount){if(modeKeyboard==1){Keyboard.print(" ");};WordCount=0;};break;
                        case 3: WordCount=0;break;
 }
}
void PrintBat(){
 ...
 if (Battery<LevelBatt1){LD.printString_12x16 ("USB ", 80, 6);}
 if (Battery>=LevelBatt1 && Battery<LevelBatt2){LD.printString_12x16 ("Bat-", 80, 6);}
 if (Battery>LevelBatt2){LD.printString_12x16 ("Bat+", 80, 6);}
   LastBattery=0;
 }
 if(!FlagPulse && !LastBattery){if (Battery>LevelBatt1 && Battery<LevelBatt2){LD.printString_12x16 ("    ", 80, 6);};LastBattery=1;}
}
void setup() {
  //Serial.begin(9600);
  pinMode(15, OUTPUT);
...
void loop() {
 ...
 doOutOff();
 if(TypeChar){
   if (((micros() - stopFlow) > ((sizeTire) / 2)) && MorseSignal == 0) {printM();index = 0;TypeChar=0;WordCount++;}
             }
 doAutoProbel();      
 PrintBat();
 bitSet(PORTB,1);
